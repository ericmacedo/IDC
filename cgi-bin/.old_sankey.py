...
try:
    ...
    document_dict = dict()
    sankey_dict = dict({"nodes":list(),"links":list()})
            ...
            for i in range(cluster_n):
                ...
                for j in cluster_docs:
                    if(j["ID"] in document_dict):
                        ...
                        document_dict[j["ID"]]["nodesId"].append(fractionId)
                    else:
                        documentId += 1
                        ...
                        for c in range(len(session_dict)):
                            if(c == sessionId):
                                document_dict[j["ID"]]["sessions"].append(dict({"clusterId": clusterId}))
                                document_dict[j["ID"]]["nodesId"].append(fractionId)
                            else:
    ...
    for doc in document_dict.values():
        if(len(doc["nodesId"]) == 1):
            continue

        fractions_copy = copy.deepcopy(doc["nodesId"])
    ...
    for i in range(fractionId + 1):
        sankey_dict["nodes"].append(dict({"id": i}))

    for index, i in enumerate(transition_dict.values()):
    ...
    response = dict({
        ...
        "lins": list(transition_dict.values()),
        "sankey": sankey_dict
    })
