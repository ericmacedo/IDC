...
function init_tsne_perplexity() {
	...
	//set default to 18
	$('#tsnePerplexitySelect option[value="30"]').attr("selected",true);
}
 ...
 */
function preprocess(update=false) {
	if (!userExists) {
		alert("No user name inserted (refresh page to insert your name).")
		return
	}
...
// function hasNewDataCallServer() {
// 	$.ajax({
// 		url: "./cgi-bin/modelHasNewData.py",
// 		type: "POST",
// 		async: true,
// 		data: {userDirectory:JSON.stringify(userDirectory), userID:JSON.stringify(userID)}, 
// 		success: function( msg ) {
// 			var status = msg['status'];
// 			hasNewData = status == "true" ? true : false
// 			if (hasNewData) {
...
$(document).ready(function(){
	if ($("#Clustering_algo_select").val() == "Vi2DC") {
		$("#numbers").attr("disabled", true);
		$("#onlyEnglish").attr("disabled", true);
		$("#lematizer").attr("disabled", true);
		$("#bigram").attr("disabled", true);
	} else {
		// $("#updateButton").attr("disabled", true);

		$("#numbers").removeAttr("disabled");
		$("#onlyEnglish").removeAttr("disabled");
		$("#lematizer").removeAttr("disabled");
		$("#bigram").removeAttr("disabled");
	}

	$("#Clustering_algo_select").change(function(event) {
		if (event.target.value == "Vi2DC") {
			// if(hasNewData) {
			// 	$("#updateButton").removeAttr("disabled");
			// }
			$("#numbers").attr("disabled", true);
