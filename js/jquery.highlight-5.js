...
jQuery.fn.highlight = function(pattern, color) {
    ...
    function innerHighlight(node, pattern) {
        ...
        if (node.nodeType === 3) { // 3 - Text node
            ...
            if (pos >= 0 && node.data.length > 0) { // .* matching "" causes infinite loop
                ...
                spanNode.className = 'highlight'; // set css **** changed by Ehsan!
                //set background color of highlight  // **** changed by Ehsan!
                $(spanNode).css( "background-color", color );
