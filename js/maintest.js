  ...
  for (var i = 0; i < documentDocumentSimilarity.length; i++) {
    if(removedDocuments[i] == false) 
    {
      ...
      if(documentClustersName.split(",").length > 1) {
        ...
        tempGeneralViewGraph += getClusterColor(documentClustersName, i) + "\"},";
      }
    }    
  };
 ...
 */
function getClusterColor(clusterName, docName) {
  // if(clusterName != "") {
  ...
  // docHash = ["Health.Other-GeneralHealthCare_96066.txt", "FoodAndDrink.Cooking&Recipes_71314.txt", "ComputersAndInternet.Software_37885.txt", 
  // "EntertainmnentAndMusic.Other-Music_62426.txt", "SocietyAndCulture.Languages_137841.txt", "EducationAndReference.Other-Education_54356.txt" ];

  var found = 0;

  // for (var i = docHash.length - 1; i >= 0; i--) {
    
  //   if (docName == docHash[i]) {
  //     found = 1
  //   }
  // }

  docHash = [184, 228, 21, 140, 274, 67]; //yahoo6
  // docHash = [236, 386, 240, 119, 394]; //ng5
  // docHash = [177, 67, 372, 15, 43, 269, 313, 125, 246, 37, 207, 196, 76]//newssperate

  for (var i = docHash.length - 1; i >= 0; i--) {
    
    if ((docName+1) == docHash[i]) {
      found = 1
    }
  }
