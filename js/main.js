...
var getGeneralViewGraphThreshold = 0.5;// 0.97, the number of edges to be extracted for the graph view
...
function getClusteringMethod() {
    ...
    success: function( msg ) { 
      ...
      if (status == "yes"){
        ...
        } else if (clusteringMethod == "Vi2DC") {
          $( "#slider0" ).slider( "enable" );
          $("#Clustering_algo_select_main").val("Vi2DC");
          $("#slider0_Textbox").removeAttr("disabled");
        }
      }
      if (status == "error"){
...
function callServer(code=0){
    ...
    } else if(CL_Method == "Vi2DC") {
        set_url = "./cgi-bin/mainWordEmbeddings.py";
    }
    
    if(CL_Method == "Vi2DC") {
      var tempServerData = new Array();
      for(var i = 0; i < serverData.length; i++) {
        tempServerData.push(new Array())
        for(var j = 0; j < serverData[i].length; j++) {
          tempServerData[i].push(serverData[i][j].trim()); // remove spaces and line breaks
        }
      }
      ...
      $.ajax({
        ...
        traditional: true,
        data: {
          clusterNumber:clusterNumber, fileName:fileName, specFileName:specFileName, fileListName:fileListName, 
          termsFileName: termsFileName, userU:userU, userDirectory:userDirectory, serverData:JSON.stringify(tempServerData), userID:JSON.stringify(userID),
          cluster_mode: code,
        ...
        success: function (msg) {
            ...
            {  
                ...
                clusterWords = eval(msg['clusterTerms']);
                documentClusterData = eval(msg["softDocs"]);
                clusterDocuments = eval(msg["clusterDocs"]);
                termClusterData = eval(msg["softTerms"]);
                clusterCloud = eval(msg["clusterCloud"]); 
                clusterKeyTerms = eval(msg["clusterKeyTerms"]);
                allWords = eval(msg["allWords"]);
                ...
                for (var i = 0; i < clusterWords.length; i++) { 
                  ...
                  for (var j = 0; j < clusterWords[i].words.length; j++) {        
                    ...
                    <li class='ui-state-default ui-sortable-handle' onmousedown=\"wordMouseDown(event)\">
                      <span class='terms'>{0}</span>
                    </li>`.format(clusterWords[i].words[j].word));
                  }
                }

                if(firstRun == true) {
                  //get and set perpelxity slider
                  asyncRequest.open( 'POST', "./users/{0}/perplexity".format(userID), false );
                  ...
                  documentsName = eval(msg["docNames"])
                  documentDocumentSimilarity = eval(msg["documentSimilarity"])
                  termDocumentSimilarity = eval(msg["termSimilarity"])
                }
                ...
                $('#button4 .button').prop('disabled', false);   
...
function getListOfFilters() {
  ...
  for (var i in filter1List) {
    ...
    $("#filter1_select").append(`
      <option class="d" value="{0}" title="{1}">{2}</option>
    `.format(filter1,filter1,filter1)
    ); 
  }
  ...
  for (var i in filter2List) {
    ...
    $("#filter2_select").append(`
      <option class="d" value="{0}" title="{1}">{2}</option>
    `.format(filter2,filter2,filter2)
    ); 
  }
...
function getClusterCloud() {
  var tempClusterCloud = new Array();

    for (var i = 0; i < clusterKeytermsOriginal.length; i++) {

      var temp = eval(clusterKeytermsOriginal[i]);

      tempClusterCloud.push({
        cluster: (serverClusterName.length > 0) ? serverClusterName[i] : "cluster{0}".format(i),
        cloud: ""
      })

      for (var j = 0; j < temp.length && j < 21; j++) {
        tempClusterCloud[i].cloud += "{0}{1}|{2}".format(
          (j == 0) ? "" : "|",
          temp[j],
          termFontSize(Math.floor(getValueOfTerm(
            (serverClusterName.length > 0) ? serverClusterName[i] : "cluster{0}".format(i),
            temp[j]
          )))
        );
      }
...
function getClusterDocuments() {
  var tempClusterDocuments = new Array();
  ...
  for (var i = 0; i < clusterDocs.length; i++) {
    ...
    tempClusterWords.push({
      cluster: (serverClusterName.length > 0) ? serverClusterName[i] : "cluster"+i,
      docs: []
    });

    for (var j = 0; j < temp.length; j++) {
      
      tempClusterDocuments[i].docs.push({
        ID: temp[j]
      });   
    }
  }

  return tempClusterDocuments;
}
...
function getClusterKeyTerms() {
  ...
  for (var i = 0; i < clusterKeytermsOriginal.length; i++) {
    ...
    var temp = eval(clusterKeytermsOriginal[i]);
    tempClusterKeyTerms.push({
      cluster: serverClusterName.length > 0 ? serverClusterName[i] : "cluster" + i,
      words: []
    });
    for (var j = 0; j < temp.length; j++) {
      tempClusterKeyTerms[0].words.push({
        word: temp[j],
        v1: serverClusterName.length > 0 ? getValueOfTerm(serverClusterName[i], temp[j]) : getValueOfTerm("cluster"+i, temp[j])
      });    
    }
  }

  return tempClusterKeyTerms;
}
...
function getClusterWords() {
  var termClusterWords = new Array();
  ...
  for (var i = 0; i < clusterKeytermsOriginal.length; i++) {
    ...
    for (var j = 0; j < temp.length; j++) {
      tempClusterWords[i].words.push({
        word: temp[j]
      })
    }
  }

  return tempClusterWords;
}
 ...
 */
function SendData2Server(code=0)
{
    $('#button4 .button').prop('disabled', true);    
...
function callSaveSession(sessionName) {
    ...
    var clusteringMethod = $("#Clustering_algo_select_main").val();
    ...
    data: { 
      ...
      documentsName:JSON.stringify(documentsName),
      sessionDescription:JSON.stringify(sessionDescription),
      silhouette:JSON.stringify(silhouette),
      documentDocumentSimilarity: JSON.stringify(documentDocumentSimilarity),
      termDocumentSimilarity:JSON.stringify(termDocumentSimilarity),
      termClusterDataString: JSON.stringify(clusteringMethod == "Vi2DC" ? "" : termClusterDataString),
      documentClusterDataString: JSON.stringify(clusteringMethod == "Vi2DC" ? "" : documentClusterDataString),
...
function getNewClusterWords() {
  var tempClusterWords = new Array();
...
function loadSession(sessionName) {
  ...
  $.ajax({
        ...
        success: function( msg ) {
            ...
            if(firstRun == true) {
              //get document-document similarity matrix
              if(data.clusteringMethod == "Vi2DC") {
                documentsName = data.documentsName;
                documentDocumentSimilarity = data.documentDocumentSimilarity;
                termDocumentSimilarity = data.termDocumentSimilarity;
              } else {
                //get the list of documents name
                asyncRequest.open( 'POST', "./users/"+userID+"/fileList", false );
                asyncRequest.send(); // send the request
...
function getGeneralViewGraph(similarityThreshold) {
  ...
  for (var i = 0; i < documentDocumentSimilarity.length; i++) {
    if(removedDocuments[i] == false) {
      ...
      //                       + "\", \"co\":\"";
      tempGeneralViewGraph.nodes.push({
        x: (tsneResult[i][0] + scale_width) * 6,
        y: (tsneResult[i][1] + scale_height) * 6,
        fixed: false,
        na: documentsName[i],
        cl: documentClustersName,
        co: (documentClustersName.split(",").length > 1) ? "black" : getClusterColor(documentClustersName)
      });

      tsneLabels[index] = documentClustersName.split(",")[0];
      index++;
    }    
  };
  ...
  if(calTsneSil) {   
    getTsneSilhouette(tsneResult, tsneLabels);
    calTsneSil = false;
  };
  ...
  for (var i = 0; i < documentDocumentSimilarity.length; i++) {
    if(removedDocuments[i] == false) 
    {
      for (var j = i; j < documentDocumentSimilarity.length; j++) {
        if(removedDocuments[j] == false) {
          if(parseFloat(documentDocumentSimilarity[i][j]) <= similarityThreshold ) {
            tempGeneralViewGraph.links.push({
              source: documentNewIndex[i],
              target: documentNewIndex[j],
              v: documentDocumentSimilarity[i][j]
            });
          }
        }
      }
  ...
  generalViewGraph2 = Object.assign(new Object(), tempGeneralViewGraph);
  ...
  // return Object.assign(new Object(), tempGeneralViewGraph);
  return JSON.parse(JSON.stringify(tempGeneralViewGraph));
}

/*
 * Get  Silhouette
 * @param tsneResult =  x and y dimensions 
 * @param tsneLabels = labels of documents
 */
function getTsneSilhouette(tsneResult, tsneLabels) {
  $.ajax({
    type: "POST",
    url: "./cgi-bin/tsneSilhouette.py",
    data: { tsneResult:JSON.stringify(tsneResult),
            tsneLabels:JSON.stringify(tsneLabels)},                
    success: function( msg ) {
...
function getForceSilhouette(forceResult, forceLables) {
  ...
  $.ajax({
    ...
    data: { tsneResult:JSON.stringify(forceResult),
            tsneLabels:JSON.stringify(forceLables)},                
    success: function( msg ) {
...
function changePerplexity() {
  ...
  $.ajax({
    ...
    traditional: true,
    data: { userDirectory:JSON.stringify(userDirectory), perplexityNew:JSON.stringify(perplexityNew), userID:JSON.stringify(userID), clusteringMethod: JSON.stringify(clusteringMethod)},
    async: false,              
    success: function( msg ) {
